...
class SyncJanischaData extends Command
{
    protected $signature = 'sync:janischa-data {--force : Force sync even if recently synced}';
    protected $description = 'Sync Janischa client data from Exness API to janischa_clients table';

    private $janischaExnessService;

    public function __construct(JanischaExnessAuthService $janischaExnessService)
    {
        parent::__construct();
        $this->janischaExnessService = $janischaExnessService;
    }

    public function handle()
    {
        $this->info('Starting Janischa data sync...');
        
        try {
            // Check if we need to sync (avoid too frequent syncing)
            if (!$this->option('force')) {
                $lastSync = JanischaClient::max('last_sync_at');
            ...
            foreach ($clients as $clientData) {
                try {
                    ...
                    Log::error('Error processing Janischa client', [
            ...
            Log::error('Janischa data sync failed', [
                'error' => $e->getMessage(),
