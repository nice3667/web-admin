...
class DebugXmClients extends Command
{
    protected $signature = 'debug:clients-xm';
    protected $description = 'Debug ข้อมูลลูกค้าทั้งหมดของ XM';

    public function handle(): int
    {
        $this->line('=== DEBUG ข้อมูลลูกค้าทั้งหมดของ XM ===');
        ...
        try {
            ...
            // 3. Unique UIDs
            $this->newLine();
            $this->info('3. ข้อมูล Unique Client UID:');
            $this->line(str_repeat('-', 40));

            $uniqueUids = DB::table('clients')
                ->where('data_source', 'XM')
                ->distinct()
                ->pluck('client_uid');

            $this->line('จำนวน Unique Client UID: ' . $uniqueUids->count());
            if ($uniqueUids->count() > 0) {
                $this->line('UID ล่าสุด: ' . $uniqueUids->first());
                $this->line('UID เก่าสุด: ' . $uniqueUids->last());
            }

            // 4. Countries
            $this->newLine();
            $this->info('4. ข้อมูลตามประเทศ:');
            $this->line(str_repeat('-', 40));
            ...
            // 6. Volume & Reward
            $this->newLine();
            $this->info('6. ข้อมูล Volume และ Reward:');
            $this->line(str_repeat('-', 40));
            ...
            // 9. Service
            $this->newLine();
            $this->info('9. ข้อมูลจาก XM API:');
            $this->line(str_repeat('-', 40));

            if (class_exists('App\\Services\\XmAuthService')) {
                try {
                    $xmService = new \App\Services\XmAuthService();
                    $this->line('XmAuthService พบและพร้อมใช้งาน');
                    if (method_exists($xmService, 'getClients')) $this->line('มี method getClients() ใน XmAuthService');
                    if (method_exists($xmService, 'getClientCount')) $this->line('มี method getClientCount() ใน XmAuthService');
                } catch (\Throwable $e) {
                    $this->warn('เกิดข้อผิดพลาดในการใช้งาน XmAuthService: ' . $e->getMessage());
                }
            } else {
                $this->line('ไม่พบ XmAuthService class');
            }

            // 10. XM folder
            $this->newLine();
            $this->info('10. ข้อมูลใน XM folder:');
            $this->line(str_repeat('-', 40));

            $xmFolderPath = base_path('XM');
            if (is_dir($xmFolderPath)) {
                $this->line('XM folder พบที่: ' . $xmFolderPath);
                $xmFiles = glob($xmFolderPath . '/*.php') ?: [];
                $this->line('จำนวนไฟล์ PHP ใน XM folder: ' . count($xmFiles));
                foreach ($xmFiles as $file) {
                    $this->line('- ' . basename($file));
                }
            } else {
                $this->line('ไม่พบ XM folder');
            }

            // 11. Backups
            $this->newLine();
            $this->info('11. ข้อมูลใน storage/backups:');
            $this->line(str_repeat('-', 40));
            ...
            $this->line('จำนวนไฟล์ backup ที่เกี่ยวกับ clients: ' . count($xmBackups));
            ...
            $this->line('จำนวนไฟล์ backup: ' . count($xmBackups));
